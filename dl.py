# core: 8, memory: 64GB, hestia, GPU: a40
...
def validate_model(model, dataloader, criterion, device):
    ...
    tn, fp, fn, tp = confusion_matrix(all_targets, binary_predictions).ravel()

    # Use raw predictions here
    roc_auc = roc_auc_score(all_targets, predictions)
...
def cross_validate_model(n_samples, measurements, labels, patient_ids, n_splits, batch_size, n_epochs, learning_rate, device):
    ...
    for i, (train_idx, val_idx) in enumerate(splits):
        ...
        for epoch in tqdm(range(n_epochs), desc=f"Split {i + 1}/{n_splits}"):
            ...
            val_loss, val_accuracy, val_precision, val_recall, val_f1, val_roc_auc, tn, fp, fn, tp, val_time = validate_model(
        ...
        print(f"Split {i + 1}/{n_splits}, best epoch at {best_epoch_metrics['epoch']}, "
              ...
              f"Val F1: {best_epoch_metrics['val_f1']:.4f}, "
              f"Val ROC AUC: {best_epoch_metrics['val_roc_auc']:.4f}, ")
...
if __name__ == "__main__":
    ...
    for input_file in input_files:
        ...
        split_metrics, avg_metrics = cross_validate_model(
            n_samples, measurements, labels, patient_ids, n_splits=N_SPLITS, batch_size=BATCH_SIZE, n_epochs=N_EPOCHS, learning_rate=LR, device=device)
        ...
        results_output = os.path.join(
            OUTPUT_PATH, f"cnn_{window_length}.json")
